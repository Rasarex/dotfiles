...
#include <string>
#include <zmq.hpp>
inline std::string trim(std::string orig)
{
  if (orig != "")
    orig.pop_back();
  return orig;
...
int main(int argc, char const *argv[])
{
  ...
  while (true)
  {
    ...
    update_message = trim(exec("playerctl metadata xesam:albumArtist 2> /dev/null"));
    update_message += " - ";
    update_message += trim(exec("playerctl metadata xesam:title 2> /dev/null"));
